...
#>
    ...
    process
    {
        ...
        [System.Text.StringBuilder] $jsonBuilder = New-Object -TypeName 'System.Text.StringBuilder'
            ...
            for ($w = 0; $w -lt $words.Count; $w++)
            {
                ...
                if ($newPosition -lt $usableWidth)
                {
                    ...
                    if ($isLastWord)
                    {
                        ...
                        [ref] $null = $jsonBuilder.Append($tabSpaces).Append($justifiedString)
                ...
                elseif ($newPosition -eq $usableWidth)
                {
                    ...
                    [ref] $null = $lineBuilder.Append($word)
                    [ref] $null = $jsonBuilder.Append($tabSpaces).Append($lineBuilder.ToString())
                ...
                else
                {
                    ...
                    if ($isLastWord)
                    {
                        ...
                        $justifiedString = ConvertTo-JustifiedString @convertToJustifiedStringParams
                        [ref] $null = $jsonBuilder.AppendLine().Append($tabSpaces).Append($justifiedString)
